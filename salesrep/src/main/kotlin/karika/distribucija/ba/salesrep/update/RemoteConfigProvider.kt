...
/**
 * Backs the silent-update mechanism: the admin publishes a new [KioskVersion] by editing these
 * parameters in the Firebase console (or via the Remote Config REST API), and every device with
 * an active [ConfigUpdateListener] picks it up in real time - no per-device push token bookkeeping
 * needed. [MIN_FETCH_INTERVAL_SECONDS] is only the fallback poll cadence for when a device's
 * real-time stream isn't connected (killed process, no listener yet, transient GMS hiccup).
 */
