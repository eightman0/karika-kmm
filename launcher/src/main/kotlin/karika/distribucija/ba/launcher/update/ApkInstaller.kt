...
/**
 * Installs an already-downloaded, checksum-verified APK (the payload app, not the launcher
 * itself) with no user prompt, relying on the launcher being Device Owner: INSTALL_REASON_POLICY
 * + USER_ACTION_NOT_REQUIRED (API 31+; on Device Owner devices below that, PackageInstaller
 * already skips confirmation without this call) are what suppress the install dialog.
 */
object ApkInstaller {
    private const val TAG = "ApkInstaller"
    private const val ACTION_INSTALL_RESULT = "karika.distribucija.ba.launcher.update.INSTALL_RESULT"
